...
const Profile = () => {
  ...
  if (!profileUser) {
    return <Typography variant="h6" color="error" textAlign="center">Không tìm thấy người dùng.</Typography>;
  }
  ...
  return (
    <Box display="flex" justifyContent="center" mt={4}>
      <Card sx={{ width: 400, p: 3, boxShadow: 3, borderRadius: 2, textAlign: "center" }}>
        <CardContent>
          <Avatar src={profileUser.avatar} sx={{ width: 120, height: 120, margin: "auto" }} />
          ...
          {user?.id === profileUser.id && (
            <Link to={`/edit-profile/${user.id}`} style={{ textDecoration: "none" }}>
